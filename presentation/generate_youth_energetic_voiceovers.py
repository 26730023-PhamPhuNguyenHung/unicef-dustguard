"""
DustGuard VN - Giọng Nam Trẻ Âm Vực Cao & Truyền Cảm Hứng (Youth & Energetic Pitch)
Tinh chỉnh âm vực cao (pitch +15Hz đến +30Hz) và nhịp điệu nhanh, dứt khoát khớp nhạc nền
"""
...
YOUTH_PROFILES = [
    {
        "id": "youth_high_bright",
        "name": "Nam Trẻ - Âm Vực Cao & Sáng Rõ",
        "pitch": "+22Hz",
        "rate": "+5%"
    },
    {
        "id": "youth_energetic_fast",
        "name": "Nam Trẻ - Năng Lượng & Dứt Khoát (Khớp Nhạc Beat)",
        "pitch": "+18Hz",
        "rate": "+8%"
    },
    {
        "id": "youth_passionate",
        "name": "Nam Trẻ - Nhiệt Huyết Truyền Lửa",
        "pitch": "+26Hz",
        "rate": "+4%"
    }
]

async def generate_profile(profile):
    p_id = profile["id"]
    out_dir = Path(__file__).parent / "output" / f"voice_{p_id}"
    out_dir.mkdir(parents=True, exist_ok=True)
    
    print(f"[*] Bắt đầu tạo: {profile['name']} (Pitch: {profile['pitch']}, Rate: {profile['rate']})...")
    ...
    merged_output = out_dir / f"full_voiceover_{p_id}.mp3"
    cmd = [
    ...
    print(f"    [OK] Đã hoàn thành: {merged_output.name}")
    return merged_output

async def main():
    print("=" * 75)
    print("RENDER TRỌN BỘ 3 TÙY CHỌN GIỌNG NAM TRẺ ÂM VỰC CAO & KHỚP NHẠC BEAT")
    print("=" * 75)
    for prof in YOUTH_PROFILES:
        await generate_profile(prof)
    print("\n[DONE] Hoàn tất toàn bộ 3 phiên bản giọng nam trẻ!")
