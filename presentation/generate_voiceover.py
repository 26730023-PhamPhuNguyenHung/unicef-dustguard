"""
DustGuard VN - Voice-Over Audio Generator
Tự động sinh audio giọng đọc cho kịch bản Video Proposal 3 phút 30 giây
"""
...
VOICES = {
    "nam_minh": "vi-VN-NamMinhNeural",  # Giọng nam trầm ấm, đĩnh đạc, chuẩn phát thanh
    "hoai_my": "vi-VN-HoaiMyNeural"     # Giọng nữ truyền cảm, thanh thoát, mạch lạc
}

async def generate_audio(voice_key="nam_minh", rate="+0%", pitch="+0Hz"):
    voice_name = VOICES.get(voice_key, "vi-VN-NamMinhNeural")
    out_dir = Path(__file__).parent / "output" / voice_key
    out_dir.mkdir(parents=True, exist_ok=True)
    
    print(f"[*] Bắt đầu sinh voiceover với giọng: {voice_name} ({voice_key})")
    ...
    for sec in SECTIONS:
        ...
        communicate = edge_tts.Communicate(sec["text"], voice=voice_name, rate=rate, pitch=pitch)
        await communicate.save(str(sec_file))
        file_list.append(sec_file)
        full_text_list.append(sec["text"])
    
    # Nối toàn bộ file thành 1 audio đầy đủ 3m30s
    ...
    merged_output = out_dir / f"full_voiceover_{voice_key}.mp3"
    ...
    print(f"[OK] Hoàn tất! File tổng hợp tại: {merged_output}")
    return merged_output

if __name__ == "__main__":
    print("=== DustGuard VN TTS Voice-Over Generator ===")
    asyncio.run(generate_audio("nam_minh", rate="-2%"))
    asyncio.run(generate_audio("hoai_my", rate="-2%"))
