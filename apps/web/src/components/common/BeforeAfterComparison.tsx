      ...
      <div className={`flex flex-col gap-3 ${className}`}>
        ...
        {beforeSha256 && (
          <div className="p-2.5 bg-surface-ground rounded-lg border border-border-subtle text-[11px] font-mono text-content-main flex flex-col sm:flex-row items-start sm:items-center justify-between gap-1">
            ...
            <span className="break-all select-all text-content-sub font-semibold">{beforeSha256}</span>
          </div>
        )}
      </div>
    );
    ...
    <div className={`space-y-3 ${className}`}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          {siteLocationText && (
            <span className="hidden sm:inline-flex items-center gap-1 text-[11px] font-medium text-content-sub">
              <MapPin className="w-3 h-3 text-content-muted" /> {siteLocationText}
            </span>
          )}
        </div>
      ...
      {viewMode === 'slider' ? (
        ...
        <div
          ref={containerRef}
          className="relative aspect-video rounded-civic-lg border border-border-subtle overflow-hidden select-none touch-none cursor-ew-resize bg-slate-900"
        ...
        >
          ...
          >
            <div className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-8 h-8 rounded-full bg-white border-2 border-primary shadow-md flex items-center justify-center text-content-main">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between text-xs">
              ...
              {beforeTimestamp && (
                <span className="text-[11px] text-content-sub flex items-center gap-1">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px] font-mono">
          {beforeSha256 && (
            <div className="p-2.5 bg-surface-ground rounded-lg border border-border-subtle space-y-0.5">
              ...
              <p className="break-all select-all text-content-sub font-medium">{beforeSha256}</p>
            </div>
          )}
          {afterSha256 && (
            <div className="p-2.5 bg-surface-ground rounded-lg border border-border-subtle space-y-0.5">
              ...
              <p className="break-all select-all text-content-sub font-medium">{afterSha256}</p>
            </div>
          )}
        </div>
      )}
