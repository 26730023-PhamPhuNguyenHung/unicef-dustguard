  ...
  public static aggregateCaseFacts(caseId: string): CaseFact[] {
    ...
    if (targetCase.source === 'COMMUNITY' || targetCase.source_report_count > 0) {
      ...
      facts.push({
        ...
        value: targetCase.description || 'Không có mô tả chi tiết',
    ...
    for (const sig of communitySignals) {
      facts.push({
        ...
        value: sig.description || sig.title || 'Không có mô tả chi tiết',
        source_type: 'COMMUNITY',
        source_id: sig.id,
    ...
    for (const ev of evidenceList) {
      ...
      facts.push({
        ...
        value: `Định dạng ${ev.mime_type || 'tệp'}, kích thước ${((ev.file_size || 0) / 1024).toFixed(1)} KB. Mã SHA-256: ${(ev.sha256 || '').substring(0, 16)}... Trạng thái toàn vẹn: ${currentIntegrity}.`,
    ...
    for (const lr of legalReviews) {
      facts.push({
        ...
        value: `${lr.summary || 'Đã thẩm tra'}${lr.legal_basis_note ? ` (Căn cứ viện dẫn: ${lr.legal_basis_note})` : ''}`,
        source_type: 'STAFF',
        source_id: lr.id,
    ...
    for (const rs of remedSubmissions) {
      facts.push({
        ...
        value: `${rs.description || 'Đã nộp phương án khắc phục'}${rs.review_note ? ` (Ý kiến nghiệm thu: ${rs.review_note})` : ''}`,
        source_type: 'STAFF',
        source_id: rs.id,
    ...
    for (const dec of decisions) {
      facts.push({
        ...
        value: `${dec.reason || 'Đã phê duyệt'}${dec.actor_name ? ` (Ký duyệt bởi: ${dec.actor_name} - ${dec.actor_role})` : ''}.`,
